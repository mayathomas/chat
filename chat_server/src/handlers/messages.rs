...
pub(crate) async fn send_message_handler() -> &'static str {
    "Hello, World!"
}

pub(crate) async fn list_message_handler() -> &'static str {
    "Hello, World!"
}
...
pub(crate) async fn upload_handler(
    ...
    while let Some(field) = multipart.next_field().await.unwrap() {
        let fieldname = field.file_name().map(|name| name.to_string());
        let (Some(filename), Ok(data)) = (fieldname, field.bytes().await) else {
        ...
        if path.exists() {
            info!("File {} already exist: {:?}", filename, path);
        } else {
